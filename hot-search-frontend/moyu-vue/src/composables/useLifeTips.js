...
export function useLifeTips() {
    const lifeTips = ref([
        ...
        {icon: "https://www.helloimg.com/i/2025/03/27/67e50dfcd3d9a.ico", title: '好好住', description: '家居装修社区', url: "https://www.haohaozhu.cn/community/discover/keting"},
    ...
    const life = 'life';
    const getLifeTips = () => {
        axios.get(`/MustModule/getMustModule/${life}`)
            .then(response => {
                console.log("获取到life:", response.data.data);
                lifeTips.value = response.data.data;
            })
            .catch(error => {
                console.log(error);
            });
